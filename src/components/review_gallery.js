  ...
  async initCruiseImages(id) {
    ...
    aux_data.forEach((data) => {
      for (let i = 0; i < data.data_array.length; i += 2) {
        const tabKey = data.data_source === 'eventFileAttachments' ? ATTACHED_IMAGES_LABEL : data.data_array[i].data_value
      ...
      <Container className='mt-2'>
        ...
        <Row className='d-flex align-items-center justify-content-between'>
          <ButtonToolbar className='mb-2 ms-1 align-items-center'>
            <span onClick={() => this.props.gotoCruiseMenu()} className='text-warning clickable'>
              {_Cruises_}
            </span>
            <FontAwesomeIcon icon='chevron-right' fixedWidth />
            <span className='text-warning'>{this.props.cruise.cruise_id || 'Loading...'}</span>
            <FontAwesomeIcon icon='chevron-right' fixedWidth />
            <ReviewDropdown cruiseID={this.props.cruise.id} activeMode={'Gallery'} />
          </ButtonToolbar>
